...
class SourceIndexer(ParseTreeVisitor):
    ...
    def visit(self, ctx):
        ...
        if len(theList) > 1:
            ...
            size = len(aggregate)/numCentroids
            ...
            numToUse = len(vects)/numCentroids
            counts = [0 for j in range(numCentroids)]
            for v in range(len(vects)):
                for d in range(len(vects[v])):
                    aggregate[(int(v/numToUse)*size)+d] += vects[v][d] 
                counts[int(v/numToUse)] += 1
                
            for v in range(len(aggregate)):
                aggregate[v] /= counts[v/size]
